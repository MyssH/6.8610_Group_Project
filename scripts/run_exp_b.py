...
from qwen_lid.experiments.exp_b import run_exp_b
from qwen_lid.paths import ensure_project_dirs


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Experiment B GSM8K prompt-structure control in no-thinking mode.")
...
def main() -> None:
    ...
    run_exp_b(config, n_samples=config["n_samples"], resume=not args.no_resume)
