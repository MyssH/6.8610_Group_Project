...
from qwen_lid.experiments.exp_a import run_exp_a
from qwen_lid.paths import ensure_project_dirs


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Experiment A degenerate-control LID checks.")
    parser.add_argument("--n-baseline", type=int, default=None, help="Number of GSM8K no-think baseline examples.")
...
def main() -> None:
    args = parse_args()
    ensure_project_dirs()
    config = load_config("exp_a")
    if args.n_baseline is not None:
        config["n_baseline"] = args.n_baseline
    ...
    run_exp_a(config, n_baseline=config["n_baseline"], resume=not args.no_resume)
