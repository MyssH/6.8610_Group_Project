...
Recent work on large language models (LLMs) shows that allocating additional inference-time computation through explicit reasoning---chain-of-thought prompting, scratchpads, and test-time scaling---can substantially improve multi-step problem solving \citep{zhang2025surveytesttimescalinglarge}. Chain-of-thought prompting alone raises PaLM 540B accuracy on GSM8K from $17.9\%$ to $56.9\%$ and GPT-3 175B from $15.9\%$ to $46.9\%$ \citep{wei2023chainofthoughtpromptingelicitsreasoning}. \emph{Why} this helps, however, remains contested \citep{dutta2024thinkstepbystepmechanisticunderstanding}: one view is that reasoning decomposes the problem into easier sub-steps \citep{wei2023chainofthoughtpromptingelicitsreasoning}; a second view is that reasoning buys more compute and exposes the model to more candidate trajectories \citep{wang2023selfconsistencyimproveschainthought,zhang2025surveytesttimescalinglarge}; a third, geometric view is that reasoning reorganizes internal representations, with intrinsic dimensionality rising early, contracting in intermediate layers, and stabilizing late \citep{valeriani2023geometryhiddenrepresentationslarge}.

Existing geometric analyses are almost always taken \emph{across transformer depth} on a fixed input, rather than along the autoregressive generation trajectory \citep{valeriani2023geometryhiddenrepresentationslarge,van_Aken_2019}. This distinction matters because explicit reasoning is not a single representation: each generated token updates context, shifts the next-token distribution, and can move the model into a different region of activation space. Work on logit lenses, future-token prediction, and trajectory views of chain-of-thought \citep{belrose2023eliciting,pal2023future,sun2026llm,zur2025road} suggests that intermediate hidden states themselves carry the information of evolving model decisions. We therefore ask whether the \emph{local} geometry of the hidden-state \emph{trajectory} during generation carries a signature of reasoning that final-answer accuracy alone cannot reveal, and we use Local Intrinsic Dimensionality (LID) \citep{NIPS2004_74934548} as the probe.

\paragraph{Research questions.} We organize the work around three questions. (\textbf{Q1}: validity) Does LID, computed on generated-token hidden states, distinguish ordinary task generation from clearly degenerate generation? (\textbf{Q2}: robustness) Is LID a property of the underlying task, or is it driven by superficial prompt structure such as irrelevant context or repetitive boilerplate? In particular, since degenerate \emph{output} has low LID (Q1), does degenerate \emph{input} also push the answer trajectory into a low-LID regime? (\textbf{Q3}: reasoning signature) When we run the same GSM8K problems in think and no-think modes on Qwen3 models of different sizes, where does the geometric signature of thinking actually live?

\paragraph{Contributions.} Our central empirical finding is that LID inside the thinking phase tracks \emph{whether the model gets the answer right}, while the simpler segment-level orderings that previous geometric narratives would predict are not stable across model scale.
...
\begin{itemize}
    ...
    \item Using a control in which the GSM8K prompt is padded with irrelevant context or repetitive filler while the task is held fixed, we show that the LID of the answer segment is essentially unchanged, even though one of the padded prompts is itself repetitive (Exp.\ B). LID is tied to the task being executed, not to surface prompt form.
    \item In a paired think versus no-think comparison (Exp.\ C) we report two findings that revise the natural ``thinking is structurally simpler'' or ``thinking is structurally richer'' stories. (\textbf{C.1}) The relative ordering of mean LID across the thinking, think-answer, and no-think-answer segments is \emph{not} stable across model scale: paired Wilcoxon tests show that the thinking-minus-answer differences flip sign between 8B and 14B. (\textbf{C.2}) When generations are stratified by correctness, a consistent signal appears: at every Qwen3 scale we tested, the thinking trajectories of correct generations have a positive median LID gap over those of incorrect generations (one-sided Mann--Whitney $U$, $p\leq 0.05$), and the gap is several times larger than the corresponding gap in either answer segment.
\end{itemize}

Taken together, the results recommend reading LID along the thinking trajectory not as a fixed property of the thinking mode, but as a geometric correlate of successful reasoning.
...
Our three experiments interlock. Experiment~A shows that LID is sensitive --- it cleanly separates degenerate from regular generation across all five Qwen3 scales. Experiment~B shows that LID is specific --- prompt-structure perturbations, including a deliberately repetitive prompt, do not depress the answer-segment LID. Experiment~C then uses LID as a probe of reasoning: at the segment level, the natural ``thinking is a distinct LID regime'' reading does not survive across model scale (C.1, signs flip across scale); but conditioning on correctness reveals that the thinking trajectories of correct answers consistently sit at higher LID than those of incorrect ones, with a much larger correctness gap than in either answer segment, and this pattern holds across every Qwen3 scale we tested (C.2). The reading is also internally consistent: pooling correct and incorrect thinking trajectories together at a single scale destroys the very signal that conditioning on correctness reveals --- which is exactly why C.1 looks scale-dependent in the first place.

\paragraph{Limitations.} Our findings are restricted to one model family (Qwen3), one task (GSM8K), and a single LID estimator with a fixed $k$. Wrong-answer samples are scarce at the largest scales (14B and 32B), where the models are usually right; tightening the per-model correctness comparison at those scales would require harder problems where the larger models also fail. The correctness analysis is correlational, not causal, so it does not yet show that intervening to push a thinking trajectory toward higher LID would improve accuracy.

\paragraph{Future work.} Natural next steps are extending the analysis to non-mathematical reasoning benchmarks, comparing LID against other trajectory-level statistics such as logit-lens uncertainty or representation curvature, and testing whether a thinking-phase LID statistic can be used at decode time as a self-monitoring or reranking signal.
